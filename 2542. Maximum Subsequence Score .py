...
class Solution:
    def maxScore(self, nums1: List[int], nums2: List[int], k: int) -> int:
        ...
        for i in range(n):
            arr[i][0], arr[i][1] = nums2[i], nums1[i]
        arr.sort(reverse=True)
        cur = [arr[i][1] for i in range(k)]
        heapq.heapify(cur)
        total = sum(cur)
        ans = total * arr[k - 1][0]
        for i in range(1, n - k + 1):
            rem = heapq.heappop(cur)
            heapq.heappush(cur, arr[i + k - 1][1])
            total = total - rem + arr[i + k - 1][1]
            ans = max(ans, total * arr[i + k - 1][0])
        return ans
...
def main():
    ...
    nums1, nums2, k = [2,1,14,12], [11,7,13,6], 3   # Output: 168
