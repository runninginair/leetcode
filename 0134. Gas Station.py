...
Medium      7294      680      Add to List      Share
...
class Solution:
    def canCompleteCircuit(self, gas: List[int], cost: List[int]) -> int:
        ...
        n, start, total = len(gas), -1, 0
        for i in range(n):
            gas[i] -= cost[i]
            total += gas[i]
            if total < 0: total, start = 0, -1
            elif start == -1: start = i
        return start

class Solution_v2:      ### T: O(n)     M: O(1)
    def canCompleteCircuit(self, gas: List[int], cost: List[int]) -> int:
        balance = output = tank = 0
        for i in range(len(cost)):
            gas[i] -= cost[i]
            balance += gas[i]
            tank += gas[i]
            if tank < 0: tank, output = 0, i + 1
        return -1 if balance < 0 else output 

def main():
    sol = Solution()
    sol = Solution_v2()

    gas, cost = [1,2,3,4,5], [3,4,5,1,2]    # Output: 3
    print(sol.canCompleteCircuit(gas, cost))

    gas, cost = [2,3,4], [3,4,3]            # Output: -1
    print(sol.canCompleteCircuit(gas, cost))

    gas, cost = [5], [4]                    # Output: 0
    print(sol.canCompleteCircuit(gas, cost))

    gas, cost = [2,0,3], [1,2,2]            # Output: 2
    print(sol.canCompleteCircuit(gas, cost))

    gas, cost = [5,1,2,3,4], [4,4,1,5,1]    # Output: 4
    print(sol.canCompleteCircuit(gas, cost))

    gas, cost = [5,8,2,8], [6,5,6,6]        # Output: 3
    print(sol.canCompleteCircuit(gas, cost))
