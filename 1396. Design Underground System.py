 ...
 * void checkIn(int id, string stationName, int t)
    * A customer with a card ID equal to id,
        checks in at the stationstationName at time t.
...
[[], [45,"a",3], [32,"aa",8], [27,"a",10],
[45,"ab",15], [27,"ab",20], [32,"b",22],
["aa","b"], ["a","ab"], [10,"a",24],
["a","ab"], [10,"ab",38], ["a","ab"]]
...
class UndergroundSystem:    # Test Passed.
    ...
    def checkOut(self, id: int, stationName: str, t: int) -> None:
        start_end = self.check_in[id][0] + "_" + stationName
        dur = t - self.check_in[id][1]
        self.check_in.pop(id)
        if start_end in self.map:
            self.map[start_end].append(dur)
        else:
            self.map[start_end] = [dur]

    def getAverageTime(self, startStation: str, endStation: str) -> float:
        return sum(self.map[startStation + "_" +  endStation]) / len(self.map[startStation + "_" + endStation])
