...
Medium      574     470     Companies

You are given a tree (i.e. a connected, undirected graph that has no cycles)
consisting of n nodes numbered from 0 to n - 1 and exactly n - 1 edges.
The root of the tree is the node 0, and each node of the tree has a label which
is a lower-case character given in the string labels
...
as nodes 4 and 5 have different labels than node 1, the answer is just 1
(the node itself).
...
class Solution:
    def countSubTrees(self, n: int, edges: List[List[int]], labels: str) -> List[int]:
        ans, adj = [1 for _ in range(n)], [[] for _ in range(n)]
        for x, y in edges:
            adj[x].append(y)
            adj[y].append(x)

        def dfs(node, parent):
            trace = [0 for _ in range(26)]
            trace[ord(labels[node]) - ord('a')] += 1
            for nxt in adj[node]:
                if nxt == parent: continue
                curr = dfs(nxt, node)
                for i in range(26):
                    trace[i] += curr[i]
            ans[node] = trace[ord(labels[node]) - ord('a')]
            return trace
...
def main():
    ...
    print(sol.countSubTrees(n, edges, labels))        # Output: [2,1,1,1,1,1,1]

    n, edges, labels = 4, [[0,1],[1,2],[0,3]], "bbbb"
    print(sol.countSubTrees(n, edges, labels))        # Output: [4,2,1,1]

    n, edges, labels = 5, [[0,1],[0,2],[1,3],[0,4]], "aabab"
    print(sol.countSubTrees(n, edges, labels))        # Output: [3,2,1,1,1]
